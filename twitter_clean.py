...
# load data from txt
sn_twitter_path = os.path.join(current_dir, 'twitter_dataset', 'full_social_network.txt')
...
# Load full label in order to get tot num of users
...
with np.printoptions(threshold=np.inf):
	...
	print(sp_A_uu_sn, file = sourceFile)
	sourceFile.close()
	print(sp_A_uu_sn)
...
file_path = os.path.join(current_dir, 'twitter_dataset', 'twitter_sn_uu_ids.pkl')
...
file_path = os.path.join(current_dir, 'twitter_dataset', 'twitter_sp_uu_sn_adj_mats.pkl')
with open(file_path, 'wb') as out_file:
